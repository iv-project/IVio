...
void fasta_read_from_stream_io2(benchmark::State & state)
{
    size_t const sequence_length = state.range(0);
    std::string fasta_file = generate_fastq_string(sequence_length);
    std::istringstream istream{fasta_file};
    ...
    for (auto _ : state)
    {
        istream.clear();
        istream.seekg(0, std::ios::beg);
        io2::seq_io::reader fin{
            .input = istream,
        };

        for (auto it = begin(fin); it != end(fin); ++it)
            benchmark::DoNotOptimize(it);
    }
...
void fasta_read_from_disk_io2(benchmark::State & state)
{
    size_t const sequence_length = state.range(0);
    std::string fasta_file = generate_fastq_string(sequence_length);
    seqan3::test::tmp_filename file_name = create_fasta_file_for(fasta_file);
    ...
    for (auto _ : state)
    {
        auto fin = io2::seq_io::reader {
            .input = file_name.get_path(),
        };
        for (auto it = begin(fin); it != end(fin); ++it)
            benchmark::DoNotOptimize(it);
    }
...
BENCHMARK(fasta_read_from_stream_io2)->Arg(50)->Arg(1000);
BENCHMARK(fasta_read_from_disk_io2)->Arg(50)->Arg(1000);
